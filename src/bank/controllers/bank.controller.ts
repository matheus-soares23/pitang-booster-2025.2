...
export class BankController {
  ...
  criarContaCorrente(@Body() body: { titular: string; saldoInicial?: number; limiteChequeEspecial?: number }) {
  ...
  criarContaPoupanca(@Body() body: { titular: string; saldoInicial?: number; taxaRendimento?: number }) {
  ...
  depositar(@Param('numero') numero: string, @Body() body: { valor: number }) {
    ...
    if (!sucesso) {
      throw new HttpException('Erro ao realizar depósito', HttpStatus.BAD_REQUEST);
    }
  ...
  sacar(@Param('numero') numero: string, @Body() body: { valor: number }) {
    ...
    if (!sucesso) {
      throw new HttpException('Erro ao realizar saque - saldo insuficiente', HttpStatus.BAD_REQUEST);
    }
